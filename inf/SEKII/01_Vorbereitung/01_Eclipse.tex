...
\begin{compactenum}
\item In Eclipse ein eigenes Java Projekt anlegen: \myPMI{File} $\rightarrow$
\myPMI{New} $\rightarrow$ \myPMI{Java Project} $\rightarrow$ Name wählen
$\rightarrow$ \myPMI{Finish} (Abbildung \ref{fig:eclipse-java-project-creation})
...
\myPMI{Team} $\rightarrow$ \myPMI{Share Project\ldots} $\rightarrow$ \myPMI{Git}
$\rightarrow$ \myPMI{Next} $\rightarrow$ Haken bei \myPMI{Use or create
repository in parent folder of project}; Haken vor dem Projekt $\rightarrow$
\myPMI{Create Repository} (Abbildung \ref{fig:connect_to_repository_1})
$\rightarrow$ \myPMI{Finish}
...
\item \myPMI{Window} $\rightarrow$ \myPMI{Show View} $\rightarrow$
\myPMI{Other\ldots} $\rightarrow$ \myPMI{Git} $\rightarrow$ \myPMI{Git
Repositories} $\rightarrow$ \myPMI{OK}

\item Im gerade erzeugten Git Repository Tab: Klick auf den kleinen Pfeil vor
dem eigenem Repository $\rightarrow$ \myPMI{Remotes} $\rightarrow$ \myPMI{Create
Remote\ldots} Haken bei \myPMI{Configure fetch} $\rightarrow$ \myPMI{OK} 
$\rightarrow$ \myPMI{Change\ldots} $\rightarrow$ Per Coyp \& Paste die URI
(https://gitlab.com/\ldots) einfügen (Abbildung
\ref{fig:connect_to_repository_2}) $\rightarrow$ \myPMI{Finish} $\rightarrow$
\myPMI{Save and Fetch} $\rightarrow$ \myPMI{OK}
...
\myPMI{Team} $\rightarrow$ \myPMI{Switch to} $\rightarrow$ \myPMI{New
branch\ldots} $\rightarrow$ \myPMI{Select\ldots} $\rightarrow$ Klick auf
\myUserInput{choose\_remote\_name/master} $\rightarrow$ \myPMI{OK} $\rightarrow$
\myPMI{branch name:} \myUserInput{master} $\rightarrow$ \myPMI{Finish}

\item Im Project Explorer: Rechtsklick auf eigenes Projekt $\rightarrow$
\myPMI{Team} $\rightarrow$ \myPMI{Remote} $\rightarrow$ \myPMI{Configure Fetch
from Upstream\ldots} $\rightarrow$ \myPMI{Save and Fetch}

\item Im Project Explorer: Rechtsklick auf eigenes Projekt $\rightarrow$
\myPMI{Team} $\rightarrow$ \myPMI{Remote} $\rightarrow$ \myPMI{Configure Push to
Upstream\ldots} $\rightarrow$ \myPMI{Advanced} $\rightarrow$ \myPMI{Add All
Branches Spec} $\rightarrow$ \myPMI{Finish} $\rightarrow$ \myPMI{Save and Push}
\end{compactenum}



%1. File --> New --> Java Project --> Name wählen --> Finish

%2. Rechtsklick auf neues Projekt im Project Explorer --> Team --> Share
%Project\ldots --> Git --> Next --> Haken bei "`Use or create
%repository in praent folder of project"; Haken vor dem Projekt -->
%Create Repository (Screenshot 1) --> Finish

%3. Window --> Show View --> Other\ldots --> Git --> Git Repositories --> OK

%4. Git Repository Tab: Klick auf kleinen Pfeil vor eigenem Repository -->
%Remotes --> Create Remote\ldots --> Haken bei "`Configure fetch" --> OK -->
%Change\ldots Per Coyp & Paste die URI (https://gitlab.com/\ldots) (Screenshot
% 2) --> Finish --> Save & Fetch --> OK

%5. Im Project Explorer: Rechtsklick auf eigenes Projekt --> Team --> Switch to
%--> New branch\ldots --> Select\ldots --> Click auf "choose_remote_name/master"
%--> OK --> branch name: "master" --> Finish

%6. Im Project Explorer: Rechtsklick auf eigenes Projekt --> Team --> Remote -->
%Configure Fetch from Upstream\ldots --> Save and Fetch

%7. Im Project Explorer: Rechtsklick auf eigenes Projekt --> Team --> Remote -->
%Configure Push to Upstream\ldots --> Advanced --> Add All Branches Spec -->
