...
Als \myPMI{Name} trägst du \myPMI{WindowBuilder for Neon} ein. Als
\myPMI{Location}
...
$\rightarrow$ \myPMI{OK} $\rightarrow$ Häkchen setzen vor \myPMI{WindowBuilder}
$\rightarrow$ \myPMI{next} $\rightarrow$ \myPMI{next} $\rightarrow$
\myPMI{Accept Licence Agreement} $\rightarrow$ \myPMI{Finish}
...
\url{http://dl.dropbox.com/u/31241540/mysql-connector-java-5.1.40-bin.jar}
...
\ldots} $\rightarrow$ wähle \myFile{mysql-connector-java-5.1.40-bin.jar} in
...
Den Pfad zu \myFile{mysql-connector-java-5.1.40-bin.jar} (oder neuere Version)
